...
<form id="search-form"><input id="query" placeholder="Search"><button id="submit" type="submit">Search</button></form>
<div id="result" class="result-container"></div>
<p><a id="download" href="/download/sample.csv" download="sample.csv">Download sample CSV</a></p>
<form id="upload-form" enctype="multipart/form-data" method="post" action="/upload"><input id="file" name="file" type="file"><button id="upload" type="submit">Upload</button></form>
<p><a href="/table">Customer table</a> <a href="/auth">Login demo</a> <a href="/page2">Page two</a> <a id="popup" href="/popup" target="_blank">Open popup</a></p>
<button id="alert" onclick="alert('Phase B dialog')">Open alert</button>
<script>document.getElementById('search-form').addEventListener('submit',async function(event){event.preventDefault();const q=document.getElementById('query').value;const result=document.getElementById('result');try{const response=await fetch('/api/search?q='+encodeURIComponent(q));if(!response.ok)throw new Error('HTTP '+response.status);const data=await response.json();result.textContent='Search result: '+data.results[0]+' Query: '+q;result.classList.add('visible');}catch(error){result.textContent='Search failed: '+error.message;result.classList.add('visible');}});</script>
</body></html>'''
