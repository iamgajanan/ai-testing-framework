...
class TestGenerator:
    ...
    def generate(self, url: str, output_path: str, browser: str = "chromium", base_url: str = "", max_pages: int = 1) -> str:
        if int(max_pages) <= 1:
            # Keep the established extraction hook for existing callers and unit tests.
            page_info = self._extract_page_info(url, browser, base_url)
            pages = [(page_info, self._relative(url, base_url))]
        else:
            pages = self._discover_pages(url, browser, base_url, max_pages=max_pages)
        all_tests: list[dict[str, Any]] = []; suite_title = "Generated Suite"
        for page_info, relative_url in pages:
            ...
            if self.client is not None: suite = self._sanitize_suite(suite, page_info, relative_url)
            for test in suite.get("tests", []):
                test["id"] = f"GEN-{len(all_tests)+1:03d}"; test["url"] = relative_url; all_tests.append(test)
    ...
    def _discover_pages(self, url: str, browser: str, base_url: str, max_pages: int = 1) -> list[tuple[dict[str, Any], str]]:
        ...
        origin = urlparse(target).netloc; queue = [target]; seen: set[str] = set(); discovered=[]
        with sync_playwright() as p:
            b = getattr(p, browser).launch(headless=True); page = b.new_page()
            try:
                while queue and len(discovered) < max_pages:
                    ...
                    seen.add(current)
                    try:
                        page.goto(current, wait_until="domcontentloaded", timeout=15000); info = page.evaluate(_DOM_EXTRACT_JS)
    ...
    def _ai_generate(self, page_info: dict[str, Any], relative_url: str) -> dict[str, Any]:
        ...
        suite = json.loads(raw); test_suite = suite.get("test_suite")
        if isinstance(test_suite, dict):
            nested_tests = test_suite.get("tests")
            if not isinstance(suite.get("tests"), list) or not suite.get("tests"): suite["tests"] = nested_tests if isinstance(nested_tests, list) else []
            suite["test_suite"] = test_suite.get("name") or test_suite.get("id") or page_info.get("title") or "Generated Suite"
        else: suite.setdefault("test_suite", page_info.get("title") or "Generated Suite")
        if not isinstance(suite.get("tests"), list): suite["tests"] = []
        return suite
    ...
    def _sanitize_suite(self, suite: dict[str, Any], page_info: dict[str, Any], relative_url: str) -> dict[str, Any]:
        ...
        known = {self._selector(x) for x in inputs + buttons + links if self._selector(x)}; file_selectors={self._selector(x) for x in inputs if x.get("type")=="file"}
        ...
        observed_text=set(page_info.get("headings", [])) | {x.get("text", "") for x in buttons+links if x.get("text")}; observed_text |= set(page_info.get("body_text", "").splitlines()); cleaned=[]
        for index,test in enumerate(suite.get("tests", []),1):
            ...
            for step in test.get("steps",[]) or []:
                ...
                if action in {"upload","set_input_files"}:
                    ...
                    if selector not in file_selectors or not value or not Path(str(value)).expanduser().is_file(): continue
                    step["action"]="upload"
                if action=="download" and selector not in download_selectors: continue
                steps.append(step)
            test["steps"]=steps; validations=[]
            for validation in test.get("validations",[]) or []:
                if not isinstance(validation,dict): continue
                vtype,expected=validation.get("type"),validation.get("expected")
                if vtype=="text_contains" and expected and expected not in observed_text: continue
                if vtype=="element_present" and validation.get("selector") not in known and validation.get("selector") not in {"body","table"}: continue
                validations.append(validation)
            test["validations"]=validations; test["error_checks"]=["console_errors"] if "console_errors" in (test.get("error_checks") or []) else []; cleaned.append(test)
    ...
    def _looks_downloadable(href: str) -> bool:
        path=str(href).lower().split("?",1)[0]; return any(path.endswith(ext) for ext in (".csv",".pdf",".json",".xlsx",".zip",".txt"))
