...
class PlaywrightEngine:
    ...
    def __init__(self, browser_name="chromium", headless=True, timeout=30000, ai_provider="none", ai_model="gpt-4o-mini", self_healing=True, healing_confidence=0.70, artifact_dir="reports"):
        self.browser_name=browser_name; self.headless=headless; self.timeout=timeout; self.artifact_dir=Path(artifact_dir)
        self.ai_locator=AIElementLocator(ai_provider, ai_model); self.self_healing=SelfHealing(ai_provider, ai_model, healing_confidence) if self_healing else None
        self.playwright=None; self.browser=None; self.context=None; self.page=None; self.console_errors=[]; self.api_errors=[]; self.downloads=[]; self.uploads=[]; self.healed_selectors=[]; self.dialogs=[]
    ...
    def open(self,url,base_url=""):
        assert self.page is not None
        target=f"{base_url.rstrip('/')}/{url.lstrip('/')}" if base_url and url.startswith("/") else url; self.page.goto(target,wait_until="domcontentloaded")
    ...
    def _set_cookie(self,cookie):
        ...
        item=dict(cookie)
        if "url" not in item and "domain" not in item: item["url"]=self.page.url
    ...
    def _login(self,value):
        ...
        self.page.locator(us).fill(str(value.get("username",""))); self.page.locator(ps).fill(str(value.get("password","")))
        success_url=value.get("success_url")
        if success_url:
            with self.page.expect_navigation(wait_until=value.get("wait_until","domcontentloaded"),timeout=self.timeout): self.page.locator(ss).click()
        else:
            self.page.locator(ss).click()
            if value.get("wait_for_load_state"): self.page.wait_for_load_state(str(value["wait_for_load_state"]))
    ...
    def run_step(self,step):
        assert self.page is not None
        action=step.action.lower().strip(); selector=step.selector; description=getattr(step,"description","") or ""; value=step.value; value_text="" if value is None else str(value); timeout=getattr(step,"timeout",self.timeout)
    ...
    def response_text(self):
        assert self.page is not None; return self.page.locator("body").inner_text()
